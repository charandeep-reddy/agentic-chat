...
export const baseTools = {
  ...
  render_chart: tool({
    description: [
      ...
      "Prefer a chart whenever a visual comparison, trend, or distribution helps the user understand the answer, but skip one for fewer than about 4 values.",
      "Pick the type from the shape of the question: line or area for change over time, bar for comparing categories, scatter for correlation between two measures, pie only for a handful of parts of one whole.",
    ].join(" "),
  ...
  render_flow: tool({
    description: [
      ...
      "Use it for processes, steps, architecture, or relationships between things.",
      ...
      'Wrap every node label in double quotes - `A["Pure execute() returns a spec"]`, not `A[Pure execute()...]`. Parentheses, brackets and commas are shape syntax to the parser, so an unquoted label containing them is a syntax error rather than a label.',
    ].join(" "),
  ...
  render_html: tool({
    description: [
      ...
      "The frame already provides the chat's font, colours and styling for headings, labels, inputs, sliders, selects, buttons and tables. Do not set a background, do not pick colours or fonts, and do not wrap the widget in a card, panel or border - it should read as part of the message, not as an embedded page. Add CSS only for layout and for the one thing genuinely specific to what you are building. For an accent or a divider use var(--accent), var(--text-muted), var(--border).",
