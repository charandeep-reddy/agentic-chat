  ...
  if (state === "done" && result) {
    return (
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-accent/25 bg-accent-soft px-4 py-3">
        ...
        <Link
          href="/memory"
          className="shrink-0 rounded-lg bg-accent px-3 py-1.5 text-[12px] font-medium text-accent-text hover:brightness-110"
        >
          Review them
        </Link>
      </div>
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className="flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-[13px] font-medium text-accent-text hover:brightness-110 disabled:opacity-50"
      >
        <IconDownload size={15} />
        {state === "busy" ? "Adding…" : "Add to my memory"}
      </button>
      {state === "error" && (
        <p className="text-[12px] text-danger">Could not install the pack. Try again.</p>
      )}
    </div>
  );
}
