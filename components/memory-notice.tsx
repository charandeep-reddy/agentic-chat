  ...
  return (
    <div className="animate-fade-up flex items-start gap-2.5 rounded-xl border border-human/25 bg-human-soft px-3 py-2.5">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[12px] font-medium text-human">
          {saved.alreadyKnown ? "Already remembered" : "Memory saved"}
        </p>
        <p className="mt-0.5 text-[13px] leading-relaxed text-text-secondary">{saved.content}</p>
      </div>
      <Link
        href="/memory"
        className="shrink-0 rounded-md px-2 py-1 text-[11px] text-text-faint underline-offset-2 hover:text-text-secondary hover:underline"
      >
        Manage
      </Link>
    </div>
